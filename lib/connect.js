var lms = require('squeezenode'), Squeezebox;
// Xbmc
...
module.exports = function(identifier){
  		...
  		Squeezebox.getPlayers(function(response){
  			...
  			if (response.ok && response.result.length){
  				response.result.forEach(function(player){
  					// add more magic?
  					sails.log.info(`Squeeze player found : ${player.name}`);
  				});
  				callback(players);
  			}
  		});
  ...
  // var connection = new Xbmc.TCPConnection({ host : host,  port : port,  verbose: true, connectNow : false });
/*
...
*/
    ...
    gladys.device.create(newDevice)
      ...
      .then(() => gladys.deviceState.createByDeviceTypeIdentifier(`${identifier}-connected`, 'lms', {value: 1}))
      .catch((err) => sails.log.error(`LMS module : Error while creating device : ${err}`));
  };

/*
  connection.onError = function(){
    sails.log.info(`LMS module : Connection error to ${host}:${port}`);
    gladys.deviceState.createByDeviceTypeIdentifier(`${identifier}-connected`, 'lms', {value: 0})
      .catch(() => {});
  };
